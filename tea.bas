...
20 POKE 55, 0 : POKE 56,200 : REM RESERVE MEMORY FOR ASM (ADJUST ACCORDINGLY)
...
261 INPUT H$
262 IF LEN(H$) = 0 THEN PRINT "HEX CANNOT BE EMPTY" : GOTO 260
263 IF LEN(H$) > 128 THEN PRINT "HEX TOO LONG.": H$ = LEFT$(S$,128) : PRINT "TRUNCATED TO 128 CHARS."
264 IF INT(LEN(H$) / 2) <> LEN(H$) / 2 THEN PRINT "HEX LENGTH MUST BE EVEN." : GOTO 260
265 IF INT(LEN(H$) / 16) <> LEN(H$) / 16 THEN PRINT "HEX MUST BE MULTIPLE OF 16." : GOTO 260
